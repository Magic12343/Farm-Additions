...
import com.avalon.farmadditions.client.util.FAAdvancements;
...
public class GrapeVineBlock extends Block implements IForgeShearable, IGrowable{
	...
	public ActionResultType onBlockActivated(BlockState state, World worldIn, BlockPos pos, PlayerEntity player, Hand handIn, BlockRayTraceResult hit) {
		...
		if (!flag && player.getHeldItem(handIn).getItem() == Items.BONE_MEAL) {
			...
			if (state.get(AGE) == 4) spawnAsEntity(worldIn, pos, new ItemStack(ItemInit.GRAPES.get(), 1));
			worldIn.setBlockState(pos, state.with(DOUBLE, false), 2);
			return ActionResultType.SUCCESS;
		} else if (flag) {
			spawnAsEntity(worldIn, pos, new ItemStack(ItemInit.GRAPES.get(), state.get(DOUBLE) ? 2 : 1));
			...
			if (player instanceof ServerPlayerEntity && player.isAlive()) {
    			...
    			if(!player.world.isRemote()) {
    				FAAdvancements.PICK_GRAPES.trigger(serverPlayer); 
    			}
    		}
