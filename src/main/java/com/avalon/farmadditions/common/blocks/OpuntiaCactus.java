...
public class OpuntiaCactus extends DeadBushBlock implements IGrowable {
	   ...
	   public OpuntiaCactus(AbstractBlock.Properties p_i49971_1_) {
	      super(p_i49971_1_);
	      this.setDefaultState(this.stateContainer.getBaseState().with(AGE, Integer.valueOf(0)));
	   }

	   public ItemStack getItem(IBlockReader worldIn, BlockPos pos, BlockState state) {
	      return  ItemStack(ItemInit.OPUNTIA_FRUIT.get());
	   }

	   private ItemStack ItemStack(Item item) {
	...
	public VoxelShape getShape(BlockState state, IBlockReader worldIn, BlockPos pos, ISelectionContext context) {
	   ...
	   public ActionResultType onBlockActivated(BlockState state, World worldIn, BlockPos pos, PlayerEntity player, Hand handIn, BlockRayTraceResult hit) {
	      ...
	      if (!flag && player.getHeldItem(handIn).getItem() == Items.BONE_MEAL) {
	         ...
	         spawnAsEntity(worldIn, pos, new ItemStack(ItemInit.OPUNTIA_FRUIT.get(), j + (flag ? 1 : 0)));
